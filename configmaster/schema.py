...
class SettingSchema:
    ...
    def __init__(self, name: str, definition: dict):
        ...
        log.debug(f"Initializing schema for setting '{name}' with definition: {definition}")
        ...
        if type_str not in SUPPORTED_TYPES:
            raise SchemaError(f"Unsupported type '{type_str}' for setting '{name}'. Supported types: {list(SUPPORTED_TYPES.keys())}")
        ...
        if not isinstance(self.nullable, bool):
             raise SchemaError(f"Schema 'nullable' flag for '{name}' must be a boolean (True or False).")
        ...
        if "default" in definition:
             ...
             log.debug(f"Setting '{name}' is nullable and has no default specified. Defaulting to None.")
        else:
             # If not nullable and no default, raise error
             raise SchemaError(f"Schema for non-nullable setting '{name}' must include a 'default' value.")

        # Help Text (Mandatory)
        self.help: str = definition.get("help", "")
        if not self.help or not isinstance(self.help, str):
             log.warning(f"Schema for '{name}' should include a non-empty 'help' string for documentation.")
        ...
        if self.options is not None:
            ...
            if not self.options:
                raise SchemaError(f"Schema 'options' for '{name}' cannot be an empty list.")
        ...
        self.min_val: typing.Optional[typing.Union[int, float]] = definition.get("min_val")
        self.max_val: typing.Optional[typing.Union[int, float]] = definition.get("max_val")

        if self.type not in [int, float]:
            if self.min_val is not None or self.max_val is not None:
                log.warning(f"'min_val'/'max_val' are only applicable to int/float types. Ignored for '{name}' (type: {self.type_str}).")
                self.min_val = None
                self.max_val = None
        else:
            if self.min_val is not None and not isinstance(self.min_val, (int, float)):
                 raise SchemaError(f"'min_val' for numeric setting '{name}' must be a number (int or float).")
            if self.max_val is not None and not isinstance(self.max_val, (int, float)):
                 raise SchemaError(f"'max_val' for numeric setting '{name}' must be a number (int or float).")
            if self.min_val is not None and self.max_val is not None and self.min_val > self.max_val:
                raise SchemaError(f"'min_val' ({self.min_val}) cannot be greater than 'max_val' ({self.max_val}) for setting '{name}'.")
            ...
            if self.options is not None and self.default_value is not None:
                ...
                if not (self.nullable and self.default_value is None):
                     if self.default_value not in coerced_options:
                          raise SchemaError(f"Default value '{self.default_value}' for setting '{name}' is not among the allowed options: {self.options}.")

        except ValidationError as e:
            raise SchemaError(f"Default value '{self.default_value}' for setting '{name}' failed validation: {e}")
    ...
    def _coerce_value(self, value: typing.Any) -> typing.Any:
        ...
        if self.type is bool:
            ...
            log.debug(f"Could not strictly coerce '{value}' (type: {type(value).__name__}) to bool. Passing original value to validation.")
        ...
        try:
            ...
            if not isinstance(value, self.type):
                ...
                if self.type not in [list, dict]:
                     log.debug(f"Attempting standard coercion for {value} to {self.type_str}")
                     return self.type(value)
            return value # Return original if already correct type or complex type
        except (ValueError, TypeError) as e:
            log.warning(f"Standard coercion failed for '{value}' to type {self.type_str}: {e}. Passing original value to validation.")
    ...
    def validate(self, value: typing.Any):
        ...
        if value is None:
            if self.nullable:
                ...
                raise ValidationError(f"Value for setting '{self.name}' cannot be None (nullable is False).")
        ...
        if not isinstance(coerced_value, self.type):
            ...
            if not (self.type is bool and isinstance(coerced_value, (int, float)) and coerced_value in [0, 1]):
                raise ValidationError(f"Value '{value}' for setting '{self.name}' must be of type '{self.type_str}', but got '{type(value).__name__}'.")
        ...
        if self.options is not None:
            ...
            if self.type is bool and isinstance(coerced_value, bool):
                ...
                if coerced_value not in allowed_options:
                     raise ValidationError(f"Value '{value}' for setting '{self.name}' is not one of the allowed options: {self.options}.")
        ...
        if self.type in [int, float]:
            if self.min_val is not None and coerced_value < self.min_val:
                raise ValidationError(f"Value '{value}' for setting '{self.name}' is below the minimum allowed value of {self.min_val}.")
            if self.max_val is not None and coerced_value > self.max_val:
                raise ValidationError(f"Value '{value}' for setting '{self.name}' is above the maximum allowed value of {self.max_val}.")
        ...
        log.debug(f"Validation successful for setting '{self.name}' with value '{value}'.")
    ...
    def to_dict(self) -> dict:
        ...
        if "default" in self._raw_definition or self.default_value is not None or (self.default_value is None and self.nullable):
