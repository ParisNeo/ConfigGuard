...
from .base import StorageHandler, LoadResult
from ..exceptions import HandlerError, EncryptionError
from ..log import log
...
class JsonHandler(StorageHandler):
    ...
    def load(self, filepath: Path) -> LoadResult:
        ...
        try:
            ...
            if self._fernet:
                ...
                decrypted_bytes = self._decrypt(raw_data) # Raises EncryptionError on failure
            ...
            try:
                 ...
                 log.error(f"JsonHandler: UTF-8 decode error in {filepath} after potential decryption: {e}")
                 ...
                 log.error(f"JsonHandler: Unexpected error during JSON parsing of {filepath}: {e}", exc_info=True)
                 raise HandlerError(f"Failed to parse JSON content from {filepath}: {e}") from e
            ...
            if isinstance(loaded_data, dict):
                ...
                if all(k in loaded_data for k in ('version', 'schema', 'values')):
                    log.debug(f"JsonHandler: Loaded 'full' structure (version, schema, values) from {filepath}")
                    ...
                    if not isinstance(loaded_values, dict):
                        raise HandlerError(f"Invalid 'values' section in full structure file {filepath} (must be a dictionary).")
                    if loaded_schema is not None and not isinstance(loaded_schema, dict):
                         raise HandlerError(f"Invalid 'schema' section in full structure file {filepath} (must be a dictionary or null).")
                    ...
                    log.debug(f"JsonHandler: Loaded simple values dictionary from {filepath}")
                    return {
                        'version': None,
                        'schema': None,
                        'values': loaded_data
                 ...
                 # If the top level isn't a dict, it's an invalid format for ConfigMaster JSON storage
                 log.error(f"JsonHandler: Root JSON element in {filepath} is not a dictionary (found {type(loaded_data).__name__}).")
                 raise HandlerError(f"Root JSON element in {filepath} must be a dictionary.")
    ...
    def save(self, filepath: Path, data: dict, mode: str = 'values') -> None:
        ...
        Args:
            ...
            data: The full data payload from ConfigMaster, containing keys
        ...
        if mode == 'full':
            ...
            if not all(k in data for k in required_keys):
                 ...
                 log.error(f"JsonHandler: Invalid data structure provided for 'full' save mode. Missing keys: {missing}")
                 raise HandlerError(f"Invalid data structure provided for 'full' save mode. Missing keys: {missing}")
            ...
            log.debug("JsonHandler: Preparing 'full' data structure for JSON serialization.")
        elif mode == 'values':
            # Check if required key is present
            if 'config_values' not in data:
                 log.error("JsonHandler: Invalid data structure provided for 'values' save mode. Missing 'config_values' key.")
                 raise HandlerError("Invalid data structure provided for 'values' save mode. Missing 'config_values' key.")
            ...
            if not isinstance(data_to_serialize, dict):
                 log.error(f"JsonHandler: 'config_values' in data payload is not a dictionary (type: {type(data_to_serialize).__name__}) for 'values' mode.")
                 raise HandlerError("'config_values' must be a dictionary for 'values' save mode.")
            log.debug("JsonHandler: Preparing 'values'-only data for JSON serialization.")
        else:
            # Should be caught by ConfigMaster, but double-check here
        ...
        try:
            ...
            try:
                 ...
                 log.error(f"JsonHandler: JSON serialization failed for data: {e}", exc_info=True)
                 ...
                 log.error(f"JsonHandler: Unexpected error during JSON serialization: {e}", exc_info=True)
            ...
            if self._fernet:
                ...
                bytes_to_write = self._encrypt(json_bytes) # Raises EncryptionError on failure
