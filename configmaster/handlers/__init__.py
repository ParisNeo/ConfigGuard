...
# Date: 30/04/2025
# Description: Initializes the handlers package for ConfigMaster.
...
def get_handler(filepath: typing.Union[str, Path], fernet: FernetInstance = None) -> StorageHandler:
    """
    ...
    log.debug(f"Handler Factory: Determining handler for file: {filepath} (extension: '{extension}')")

    handler_class: typing.Optional[typing.Type[StorageHandler]] = HANDLER_MAP.get(extension)
    ...
    if handler_class:
        try:
            ...
            log.debug(f"Handler Factory: Instantiated handler: {handler_class.__name__} {encryption_status} encryption support.")
            return handler_instance
        except Exception as e:
             # Catch potential errors during handler initialization
             log.error(f"Handler Factory: Failed to instantiate handler {handler_class.__name__} for extension '{extension}': {e}", exc_info=True)
             raise HandlerError(f"Failed to initialize handler for extension '{extension}': {e}") from e
    else:
        # No handler class was found in the map for this extension
        supported_extensions = list(HANDLER_MAP.keys())
        log.error(f"Handler Factory: No storage handler found for file extension '{extension}'. Supported extensions: {supported_extensions}")
        raise HandlerError(f"Unsupported configuration file extension: '{extension}'. Supported extensions: {supported_extensions}")
